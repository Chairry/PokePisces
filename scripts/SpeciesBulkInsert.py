...
#Also any names containing ( will be ignored
ignoreList = ["Happea", "Nincada", "Ninjask", "Shedinja", "Seedot", "Nuzleaf", "Shiftry", "Croagunk", "Toxicroak", "Pawniard", "Bisharp", "Kingambit", "Spinda", "Gligar", "Gliscor", "Grimer", "Muk", "Luvdisc", "Marinaroc", "Mettatoll", "Papypus♂", "Lunatone", "Solrock", "Groudon", "Kyogre"]
url = f''

with open("sheets_url.txt", "r") as speciesHeader:
    data = speciesHeader.readlines()
    url = data[0]
    
sheetData = pandas.read_csv(url)
#Unnamed: 10 is the Type2 column
pokemon = sheetData[["#", "Name:", "Icon Pal:", "Category:", "Type:", "Unnamed: 10", "Abilities:", "HP", "ATK", "DEF", "SPA", "SPD", "SPE", "Dex Entry:", "Gender Ratio (M:F):", "Egg Group:"]]


for sheetIndex in pokemon.iloc[100:281,].index:
    if pokemon["Name:"][sheetIndex] not in ignoreList and '(' not in pokemon["Name:"][sheetIndex]:
        prevMon = newMon
        newMon = pokemon["Name:"][sheetIndex]
        prevMonUnSafe = prevMon
        newMonUnsafe = newMon
        prevMon = prevMon.replace("'", "").replace("-", "")
        newMon = newMon.replace("'", "").replace("-", "")
        ...
        with open("../src/data/graphics/pokemon.h", "r+") as pokemonHeader:
            ...
            for line in data:
                ...
                if m := re.search(r"(\d{1,9})"+prevMon.lower()+r"\/footprint.1bpp", line):
                    line += "const u32 gMonFrontPic_"+newMon+"[] = INCBIN_U32(\"graphics/pokemon/" + str(pokemon["#"][sheetIndex].astype(int)) + newMon.lower() + "/front.4bpp.lz\");\n"
                    line += "const u32 gMonPalette_"+newMon+"[] = INCBIN_U32(\"graphics/pokemon/" + str(pokemon["#"][sheetIndex].astype(int)) + newMon.lower() + "/normal.gbapal.lz\");\n"
                    line += "const u32 gMonBackPic_"+newMon+"[] = INCBIN_U32(\"graphics/pokemon/" + str(pokemon["#"][sheetIndex].astype(int)) + newMon.lower() + "/back.4bpp.lz\");\n"
                    line += "const u32 gMonShinyPalette_"+newMon+"[] = INCBIN_U32(\"graphics/pokemon/" + str(pokemon["#"][sheetIndex].astype(int)) + newMon.lower() + "/shiny.gbapal.lz\");\n"
                    line += "const u8 gMonIcon_"+newMon+"[] = INCBIN_U8(\"graphics/pokemon/" + str(pokemon["#"][sheetIndex].astype(int)) + newMon.lower() + "/icon.4bpp\");\n"
                    line += "const u8 gMonFootprint_"+newMon+"[] = INCBIN_U8(\"graphics/pokemon/" + str(pokemon["#"][sheetIndex].astype(int)) + newMon.lower() + "/footprint.1bpp\");\n"
                    newData.append(line)
                else:
                    newData.append(line)
            #seek and truncate to overwrite file's content
        ...
        with open("../src/data/text/species_names.h", "r+") as speciesNamesHeader:
            ...
            for line in data:
                newLine = line.split()
                if "[SPECIES_"+prevMon.upper() in line:
                    line += "    [SPECIES_"+newMon.upper()+"] = _(\""+newMonUnsafe+"\"),\n"
                    newData.append(line)
                else:
                    newData.append(line)
            #seek and truncate to overwrite file's content
        ...
        with open("../src/data/pokemon/pokedex_text.h", "r+", encoding="utf8") as pokedexTextHeader:
            ...
            dexEntry = pokemon["Dex Entry:"][sheetIndex].splitlines()
        ...
        with open("../src/data/pokemon/pokedex_entries.h", "r+") as pokedexEntriesHeader:
            ...
            for line in data:
                ...
                if "[NATIONAL_DEX_"+prevMon.upper() in line:
                    ...
                    line += "    [NATIONAL_DEX_"+newMon.upper()+"] =\n"
                    line += "    {\n"
                    line += "        .categoryName = _(\"" + pokemon["Category:"][sheetIndex] + "\"),\n"
                    line += "        .height = 999,\n"
                    line += "        .weight = 999,\n"
        ...
        with open("../src/data/pokemon/species_info.h", "r+") as speciesInfoHeader:
            ...
            for line in data:
                ...
                if "[SPECIES_"+prevMon.upper() in line:
                    ...
                    type1 = pokemon["Type:"][sheetIndex]
                    type2 = pokemon["Unnamed: 10"][sheetIndex]
                    #if type2 is blank, pandas reads the value in as NaN, which is, frustratingly, not a string
                    type2 = type2 if isinstance(type2, str) else type1
                    
                    line += "\n"
                    line += "    [SPECIES_"+newMon.upper()+"] =\n"
                    line += "    {\n"
                    line += "        .baseHP        = " + str(pokemon["HP"][sheetIndex]) + ",\n"
                    line += "        .baseAttack    = " + str(pokemon["ATK"][sheetIndex]) + ",\n"
                    line += "        .baseDefense   = " + str(pokemon["DEF"][sheetIndex]) + ",\n"
                    line += "        .baseSpeed     = " + str(pokemon["SPE"][sheetIndex]) + ",\n"
                    line += "        .baseSpAttack  = " + str(pokemon["SPA"][sheetIndex]) + ",\n"
                    line += "        .baseSpDefense = " + str(pokemon["SPD"][sheetIndex]) + ",\n"
                    line += "        .types = { " + typesDict[type1] + "," + typesDict[type2] + " },\n"
                    line += "        .catchRate = 100,\n"
                    line += "        .expYield = 100,\n"
        ...
        #create pokemon graphics folder and fill with dummy data.  Ensures make works without adding extra steps.
        shutil.copytree("../graphics/pokemon/113happea", "../graphics/pokemon/" + str(pokemon["#"][sheetIndex].astype(int)) + newMon.lower())
