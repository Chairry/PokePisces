    ...
    else
    {
        ...
        else
        {
            ...
            case ITEM_NEST_BALL:
                ...
                //((41 - Pok?mon's level) ??申?申??申?申 10)?? if Pok?mon's level is between 1 and 29, 1?? otherwise.
                if (gBattleMons[gBattlerTarget].level < 30)
                    ballMultiplier = 410 - (gBattleMons[gBattlerTarget].level * 10);
            #elif B_NEST_BALL_MODIFIER == GEN_5
                //((41 - Pok?mon's level) ??申?申??申?申 10)??, minimum 1??
                if (gBattleMons[gBattlerTarget].level < 31)
                    ballMultiplier = 410 - (gBattleMons[gBattlerTarget].level * 10);
            #else
                //((40 - Pok?mon's level) ??申?申??申?申 10)??, minimum 1??
