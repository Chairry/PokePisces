...
static bool32 AccuracyCalcHelper(u16 move)
{
    ...
    if ((gBattleMons[gBattlerTarget].status2 & STATUS2_INFATUATION) && move == MOVE_SWEET_KISS)
    {
        JumpIfMoveFailed(7, move);
        return TRUE;
    }
        ...
        case MOVEEND_RECOIL:
                ...
                && gBattleScripting.savedDmg != 0) // Some checks may be redundant alongside this one
            {
                ...
                case EFFECT_RECOIL_33_STATUS: // Flare Blitz - can burn, Volt Tackle - can paralyze
                    ...
                    effect = TRUE;
                    break;
                case EFFECT_RECOIL_50_STATUS: // Flare Blitz - can burn, Volt Tackle - can paralyze
                    gBattleMoveDamage = max(1, gBattleScripting.savedDmg / 3);
                    BattleScriptPushCursor();
                    gBattlescriptCurrInstr = BattleScript_MoveEffectRecoilWithStatus;
                    effect = TRUE;
...
static void Cmd_various(void)
{
    ...
    case VARIOUS_SUCKER_PUNCH_CHECK:
    {
        ...
        if (gProtectStructs[gBattlerTarget].obstructed)
            ...
            gBattlescriptCurrInstr = cmd->failInstr;
        else if (gCurrentMove == MOVE_UPPER_HAND && GetChosenMovePriority(gBattlerTarget) <= 0)
            gBattlescriptCurrInstr = cmd->failInstr;
        else
            gBattlescriptCurrInstr = cmd->nextInstr;
        return;
    ...
    else
    {
        ...
        else
        {
            ...
            case ITEM_NEST_BALL:
                ...
                //((41 - Pok?mon's level) ????申?申??申?申???申?申??申?申????申?申??申?申???申?申??申?申 10)?? if Pok?mon's level is between 1 and 29, 1?? otherwise.
                if (gBattleMons[gBattlerTarget].level < 30)
                    ballMultiplier = 410 - (gBattleMons[gBattlerTarget].level * 10);
            #elif B_NEST_BALL_MODIFIER == GEN_5
                //((41 - Pok?mon's level) ????申?申??申?申???申?申??申?申????申?申??申?申???申?申??申?申 10)??, minimum 1??
                if (gBattleMons[gBattlerTarget].level < 31)
                    ballMultiplier = 410 - (gBattleMons[gBattlerTarget].level * 10);
            #else
                //((40 - Pok?mon's level) ????申?申??申?申???申?申??申?申????申?申??申?申???申?申??申?申 10)??, minimum 1??
