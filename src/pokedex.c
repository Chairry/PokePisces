...
static const struct SearchOptionText sDexSearchTypeOptions[NUMBER_OF_MON_TYPES + 1] = // + 2 for "None" and terminator, - 1 for Mystery
{
    ...
    {gText_DexEmptyString, gTypeNames[TYPE_RELIC]},
    {},
};
...
static void Task_ExitCaughtMonPage(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        ...
        LoadHueShiftedMonPalette(lzPaletteData, OBJ_PLTT_ID(paletteNum), PLTT_SIZE_4BPP, personality);
        DestroyTask(taskId);
    }
}
...
u16 CreateMonSpriteFromNationalDexNumber(u16 nationalNum, s16 x, s16 y, u16 paletteSlot)
{
    ...
    return CreateMonPicSprite(nationalNum, SHINY_ODDS + 100000, GetPokedexMonPersonality(nationalNum), TRUE, x, y, paletteSlot, TAG_NONE);
}
