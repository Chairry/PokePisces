...
bool8 CheckRelicanthWailord(void)
{
    ...
    // First comes Wailord
    if (GetMonData(&gPlayerParty[0], MON_DATA_SPECIES_OR_EGG, 0) == SPECIES_SHUCKLE)
    {
        CalculatePlayerPartyCount();
        // Last comes Relicanth
        if (GetMonData(&gPlayerParty[gPlayerPartyCount - 1], MON_DATA_SPECIES_OR_EGG, 0) == SPECIES_BLASTOISE)
            return TRUE;
    }
    return FALSE;
}
