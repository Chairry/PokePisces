...
u8 CheckMoveLimitations(u32 battler, u8 unusableMoves, u16 check)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        // Choice Items
            ...
            unusableMoves |= gBitTable[i];
        // Assault Vest
        else if (check & MOVE_LIMITATION_ASSAULT_VEST && ((holdEffect == HOLD_EFFECT_SPECTRAL_IDOL) || (holdEffect == HOLD_EFFECT_ASSAULT_VEST) || (holdEffect == HOLD_EFFECT_BATTLE_HELM)) && IS_MOVE_STATUS(gBattleMons[battler].moves[i]) && gBattleMons[battler].moves[i] != MOVE_ME_FIRST)
            unusableMoves |= gBitTable[i];
        // Gravity
...
static inline u32 CalcMoveBasePower(u32 move, u32 battlerAtk, u32 battlerDef, u32 abilityDef, u32 weather)
{
    ...
    case EFFECT_HEAVY_CANNON:
        if (gBattleMons[battlerAtk].statStages[i] > DEFAULT_STAT_STAGE) // Stat is increased.
            basePower += (gBattleMons[battlerAtk].statStages[STAT_DEF] - DEFAULT_STAT_STAGE) + (gBattleMons[battlerAtk].statStages[STAT_SPDEF] - DEFAULT_STAT_STAGE);
        break;
...
{
    if ((gBattleMoves[move].effect == EFFECT_COLLISION_COURSE || gBattleMoves[move].effect == EFFECT_GIANTS_SPEAR) && typeEffectivenessModifier >= UQ_4_12(2.0))
