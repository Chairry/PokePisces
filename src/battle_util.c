...
void PrepareStringBattle(u16 stringId, u32 battler)
{
              ...
              && !(gBattleScripting.stickyWebStatDrop == 1 && gSideTimers[targetSide].stickyWebBattlerSide == targetSide)) // Sticky Web must have been set by the foe
    {
        ...
        if (targetAbility == ABILITY_DEFIANT)
            ...
            || stringId == STRINGID_PKMNCUTSDEFENCEWITH
            || stringId == STRINGID_PKMNCUTSSPDEFENCEWITH)   
...
u32 TrySetCantSelectMoveBattleScript(u32 battler)
{
    ...
    if ((GetBattlerAbility(battler) == ABILITY_GORILLA_TACTICS || GetBattlerAbility(battler) == ABILITY_ONE_WAY_TRIP) && *choicedMove != MOVE_NONE
...
u32 CalcMoveBasePowerAfterModifiers(u32 move, u32 battlerAtk, u32 battlerDef, u32 moveType, bool32 updateFlags, u32 atkAbility, u32 defAbility, u32 holdEffectAtk, u32 weather)
{
    ...
    if (IsAbilityOnField(ABILITY_FALLING) && atkAbility != ABILITY_FALLING 
        && (IS_MOVE_SPECIAL(gCurrentMove) || IS_MOVE_PHYSICAL(gCurrentMove) ))
        modifier = uq4_12_multiply(modifier, UQ_4_12(0.75));
...
bool32 IsBattlerAffectedByHazards(u32 battler, bool32 toxicSpikes)
{
    ...
    || ((gBattleWeather & B_WEATHER_HAIL || gBattleWeather & B_WEATHER_SNOW) && GetBattlerAbility(battler) == ABILITY_SNOW_CLOAK)) {
        ret = FALSE;
    }
